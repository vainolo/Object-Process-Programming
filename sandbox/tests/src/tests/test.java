...
class Challenge {
    ...
    private void wrongLogIfRequired(String text) {
        if (log.isDebugEnabled()) {
            log.debug(text);
        }
    }
    ...
    private void doSomething_betterSolution(Collection<String> words) {
        logIfRequired("doing work on ", ";", words);
        doWork(words);
    }

    private void doSomething_otherSolution(Collection<String> words) {
