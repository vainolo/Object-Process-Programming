...
public class OPMNodeEditPolicy extends ComponentEditPolicy {
   ...
   * When a {@link OPMThing} node is deleted, there is special treatment for
   * structural links that start and end at this node. If this node is source
   * for a structural link, the {@link OPMStructuralLinkAggregator} of this link
   * must be deleted. Also if this node is the target of the only outgoing link
   * of a {@link OPMStructuralLinkAggregator}, the aggregator must be deleted.
   * 
  ...
  private CompoundCommand createRecursiveDeleteNodeCommand(OPMNode nodeToDelete) {
    ...
    for(OPMLink outgoingStructuralLink : analyzer.findOutgoingStructuralLinks(nodeToDelete)) {
    ...
    // aggregator.
    for(OPMLink incomingStructuralLink : analyzer.findIncomingStructuralLinks(nodeToDelete)) {
