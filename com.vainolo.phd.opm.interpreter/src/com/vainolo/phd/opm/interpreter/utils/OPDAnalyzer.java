...
public final class OPDAnalyzer {
  ...
  public static List<OPMProcess> getNextProcessesToExecute(final DirectedAcyclicGraph<OPMProcess, DefaultEdge> opdDag,
  ...
  public static Set<Parameter> getIncomingParameters(final OPMProcess process) {
    return getConnectedObjectsByLinkFilter(process, OPMProceduralLinkFilter.incomingFilter);
  }

  public static Set<Parameter> getOutgoingParameters(final OPMProcess process) {
    ...
    for(OPMProceduralLink link : process.getIncomingProceduralLinks())
      if(filter.filter(link.getKind()))
        if(link.getSource() instanceof OPMObject)
          parameters.add(new Parameter(link.getCenterDecoration(), (OPMObject) link.getSource(), link.getKind()));
        else
          parameters.add(new Parameter(link.getCenterDecoration(), (OPMObject) link.getTarget(), link.getKind()));
    return parameters;
  }
}
