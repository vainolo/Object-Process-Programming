 ...
 * 
 */
public class OPMInZoomedProcessExecutableInstance_Old extends OPMAbstractProcessInstance implements OPMProcessInstance {

  private static final Logger logger = SimpleLoggerFactory.createLogger(OPMInZoomedProcessExecutableInstance_Old.class
      .getName());
   ...
   */
  public OPMInZoomedProcessExecutableInstance_Old(OPMObjectProcessDiagram opd, OPDAnalyzer analyzer) {
    this.opd = opd;
  ...
  private boolean isInstanceReady(OPMProcessInstance instance) {
    for(OPMProceduralLink link : analyzer.findIncomingProceduralLinks(executionState.getProcess(instance))) {
      OPMObjectInstance variable = heap.getVariable(analyzer.getObject(link));
      if(variable == null) {
        logInfo(OPMStrings.INSTANCE_NOT_READY_PARAMETER, executionState.getProcess(instance).getName(), analyzer
        ...
        if(OPMState.class.isInstance(link.getSource())) {
          if(!valueAnalyzer.isObjectInstanceInState(variable, OPMState.class.cast(link.getSource()))) {
            logInfo(OPMStrings.INSTANCE_NOT_READY_PARAMETER_STATE, executionState.getProcess(instance).getName(),
