...
import static com.vainolo.phd.opp.utilities.OPMStrings.*;
...
import com.vainolo.phd.opp.utilities.OPMStrings;
...
import com.vainolo.phd.opp.interpreter.OPMObjectInstanceValueAnalyzer;
import com.vainolo.phd.opp.interpreter.OPMParameter;
import com.vainolo.phd.opp.interpreter.OPMProcessExecutionResult;
import com.vainolo.phd.opp.interpreter.OPMProcessInstance;
import com.vainolo.phd.opp.interpreter.OPMProcessInstanceFactory;
import com.vainolo.phd.opp.interpreter.OPMProcessInstanceHeap;
import com.vainolo.phd.opp.interpreter.inzoomedprocessinstance.OPMInZoomedProcessInstanceHeap.OPMHeapObserver;
import com.vainolo.phd.opp.interpreter.utils.OPDExecutionAnalyzer;
...
public class OPMInZoomedProcessExecutableInstance extends OPMAbstractProcessInstance implements OPMProcessInstance {
   ...
   */
  public OPMInZoomedProcessExecutableInstance(OPPObjectProcessDiagram opd, OPDAnalyzer analyzer) {
    this.opd = opd;
    this.analyzer = analyzer;
    this.executionAnalyzer = new OPDExecutionAnalyzer();
    this.valueAnalyzer = new OPMObjectInstanceValueAnalyzer();
    this.heap = new OPMInZoomedProcessInstanceHeap(valueAnalyzer, analyzer);
  ...
  protected void executing() throws Exception {
    ...
    final Map<OPMProcessInstance, OPPProcess> mapping = Maps.newHashMap();
    ...
    while(Sets.union(P_ready, P_executing).size() > 0) {
      ...
      Set<OPPProcess> P_executingProcesses = Sets.newHashSet(Collections2.transform(P_executing,
          new Function<OPMProcessInstance, OPPProcess>() {
            @Override
            public OPPProcess apply(OPMProcessInstance input) {
  ...
  private boolean objectValueTriggersEvent(OPPProceduralLink link, OPMObjectInstance objectInstance) {
  ...
  public List<OPMParameter> getIncomingParameterNames() {
    List<OPMParameter> incomingParameters = Lists.newArrayList();
    Collection<OPPObject> parameters = analyzer.findIncomingParameters(getOpd());
    for(OPPObject object : parameters) {
      incomingParameters.add(new OPMParameter(object.getName(), object.isCollection()));
    }
  ...
  public List<OPMParameter> getOutgoingParameterNames() {
    List<OPMParameter> outgoingParameters = Lists.newArrayList();
    Collection<OPPObject> parameters = analyzer.findOutgoingParameters(getOpd());
    for(OPPObject object : parameters) {
      outgoingParameters.add(new OPMParameter(object.getName(), object.isCollection()));
    }
